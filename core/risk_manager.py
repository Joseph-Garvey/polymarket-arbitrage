...
class RiskManager:
    ...
    def check_order(self, order: Order) -> bool:
        ...
        if self.state.kill_switch_triggered:
            logger.warning(
                f"Order rejected: kill switch triggered ({self.state.kill_switch_reason})"
            )
            return False
        ...
        if projected_market_exposure > self.config.max_position_per_market:
            logger.warning(
                ...
                f"{projected_market_exposure:.2f} > {self.config.max_position_per_market}"
            )
            return False
    ...
    def update_from_fill(self, trade: Trade) -> None:
        ...
        # Check if trade is part of a multi-leg group (hedged)
        strategy_tag = getattr(trade, "strategy_tag", "")
    ...
    def get_summary(self) -> dict:
        ...
        return {
            ...
            "utilization_pct": (
                self.state.global_exposure / self.config.max_global_exposure * 100
                if self.config.max_global_exposure > 0
                else 0
            ),
            ...
            "markets_with_exposure": len(
                [m for m, e in self._market_exposure.items() if e > 0]
            ),
