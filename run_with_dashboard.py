...
class TradingBotWithDashboard:
    ...
    async def _start_kalshi_monitoring(self) -> None:
        ...
        async with self.kalshi_client:
            ...
            for i in range(30):  # Wait up to 30 seconds
                ...
                if poly_count >= 50:
                    logger.info(
                        f"Got {poly_count} Polymarket markets - starting matching!"
                    )
                    break
            ...
            if self.data_feed and self._kalshi_markets:
                ...
                logger.info(
                    f"Starting background matching: {len(polymarket_markets)} Polymarket x {len(self._kalshi_markets)} Kalshi"
                )
    ...
    async def _run_matching_background(self, polymarket_markets: list) -> None:
        ...
        try:
            ...
            # Start arbitrage monitoring now that we have matched pairs
            if self._matched_pairs:
                asyncio.create_task(self._monitor_cross_platform_arb())
    ...
    async def _monitor_cross_platform_arb(self) -> None:
        """
        Continuously check matched market pairs for arbitrage opportunities.

        For each matched pair:
        1. Fetch the Polymarket order book from the live data feed
        2. Fetch the Kalshi order book via the Kalshi client
        3. Call check_arbitrage() and push any opportunity to the dashboard
        """
        logger.info(
            f"Starting cross-platform arb monitor for {len(self._matched_pairs)} pairs"
        )

        arb_cycle = 0
        while self._running and self.kalshi_client and self._matched_pairs:
            arb_cycle += 1
            opps_this_cycle = 0
            ...
            try:
                async with self.kalshi_client:
                    for pair in self._matched_pairs:
                        ...
                        # Polymarket order book — already live in the data feed
                        poly_ob = (
                        ...
                        # Kalshi order book — fetch on-demand
                        try:
                        ...
                        if opp:
                            opps_this_cycle += 1
                            dashboard_state.cross_platform["arb_opportunities"] = (
                                dashboard_state.cross_platform.get(
                                    "arb_opportunities", 0
                                )
                                + 1
                            )
                            logger.info(
                                f"🎯 Cross-platform arb: {opp.buy_platform} → {opp.sell_platform} "
                                f"{opp.token} edge={opp.edge_pct:.2%}"
                            )

                        # Throttle every pair to avoid hammering Kalshi rate limits
                        await asyncio.sleep(0.1)

                if arb_cycle % 10 == 1:
                    logger.info(
                        f"Cross-platform arb cycle {arb_cycle}: "
                        f"{opps_this_cycle} opportunities this cycle"
                    )

                # Re-check all pairs every 30 seconds
                await asyncio.sleep(30)

            except asyncio.CancelledError:
                break
            except Exception as e:
                logger.error(f"Cross-platform arb monitor error: {e}")
